"""Account manager for /cro-report skill (standalone mode).
...
def cmd_set_oauth_client(args):
    try:
        ...
        print(f"\nReady to authenticate. Run /cro-report in Claude Code.")
...
def cmd_list(_args):
    ...
    if oauth.get("exists"):
        ...
        print("  Option 1: cài /cro-setup → cro-report sẽ reuse")
        print("  Option 2: python manage_accounts.py set-oauth-client <path>\n")

    if not profiles:
        print("No profiles yet. Run /cro-report or /cro-setup in Claude Code to add one.")
        return
...
def main():
    parser = argparse.ArgumentParser(
        description="/cro-report — Account Manager",
