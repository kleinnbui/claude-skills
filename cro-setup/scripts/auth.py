"""OAuth2 flow + unified credential loader for cro-setup.

Two-phase flow (sandbox-friendly):
  - generate_auth_url(): returns URL for user to open in browser, no local server
  - complete_auth(): exchanges code from pasted redirect URL for token

Scopes:
  - tagmanager.edit.containers  (create/update DLVs, triggers, tags, versions)
  - tagmanager.readonly         (list accounts/containers/workspaces)
  - analytics.edit              (create custom dimensions, metrics, key events)
  - analytics.readonly          (list properties)
"""
...
def generate_auth_url(profile_name: str) -> dict:
    ...
    return {
        ...
        "instructions": (
            ...
            f"2. Đăng nhập Google account có quyền GTM Edit + GA4 Edit\n"
...
def discover_gtm_containers(creds) -> list[dict]:
    """List all GTM containers (Web type) the user has Edit access to."""
    ...
    try:
        ...
        for account in accounts_resp.get("account", []):
            try:
                containers_resp = service.accounts().containers().list(
                    parent=account["path"]
                ).execute()
                for c in containers_resp.get("container", []):
                    if "web" not in (c.get("usageContext") or []):
                        continue
                    out.append({
                        ...
                        "public_id": c.get("publicId", ""),  # e.g. GTM-XXXXXXX
                        "path": c["path"],
                    })
            except Exception as e:
                print(f"Warning: skipped GTM account '{account.get('name', account['accountId'])}' — {e}", flush=True)
    except Exception as e:
        print(f"Warning: could not list GTM containers: {e}", flush=True)
    return out


def discover_gtm_workspaces(creds, container_path: str) -> list[dict]:
    """List workspaces for a container. The 'Default Workspace' is the entry point."""
    from googleapiclient.discovery import build
            ...
            try:
                ...
                for prop in props_resp.get("properties", []):
                    ...
                    properties.append({
                        ...
                        "website_url": "",  # not directly on property in v1beta
                        "measurement_id": measurement_id,
                        "account": account.get("displayName", account["name"]),
                    })
            except Exception as e:
                print(f"Warning: skipped GA4 account '{account.get('displayName', account['name'])}' — {e}", flush=True)
    except Exception as e:
...
def _fetch_ga4_measurement_id(service, property_name: str) -> str:
    """Return first WEB data stream measurement ID for a property, or ''."""
    try:
